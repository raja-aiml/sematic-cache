...
func NewContextChain(maxLen int) *ContextChain {
	return &ContextChain{maxLen: maxLen, messages: make([]embedding.ChatMessage, 0, maxLen)}
}

// Add appends a message, evicting the oldest if over capacity.
func (cc *ContextChain) Add(msg embedding.ChatMessage) {
...
func (cc *ContextChain) Messages() []embedding.ChatMessage {
	out := make([]embedding.ChatMessage, len(cc.messages))
...
func (a *Agent) AddUser(text string) {
	a.Chain.Add(embedding.ChatMessage{Role: "user", Content: text})
}

// AddAssistant appends an assistant message to the context.
func (a *Agent) AddAssistant(text string) {
	a.Chain.Add(embedding.ChatMessage{Role: "assistant", Content: text})
}

// Chat sends the current context to the provided OpenAI client and
// records the assistant's response in the context chain.
func (a *Agent) Chat(ctx context.Context, client *embedding.Client, opts embedding.ChatOptions) (string, error) {
	// Prepend a system message for expert ID if set
	messages := a.Chain.Messages()
	if a.ExpertID != "" {
		systemMsg := embedding.ChatMessage{Role: "system", Content: a.ExpertID}
		messages = append([]embedding.ChatMessage{systemMsg}, messages...)
	}
