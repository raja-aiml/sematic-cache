...
func TestContextChainEviction(t *testing.T) {
	...
	cc.Add(embedding.ChatMessage{Role: "user", Content: "m1"})
	cc.Add(embedding.ChatMessage{Role: "user", Content: "m2"})
	cc.Add(embedding.ChatMessage{Role: "user", Content: "m3"})
	msgs := cc.Messages()
	if len(msgs) != 2 {
...
func TestOrchestratorRouteNotFound(t *testing.T) {
	orc := NewOrchestrator(func(s string) string { return "x" }, &embedding.Client{}, embedding.ChatOptions{Model: "gpt-3.5-turbo"})
